        ...
        try {
            ...
            if (request.method === 'POST') {
                ...
                if (planId === undefined || confirmation === undefined) {
                    // No project files changed since the selected turn: allow a
                    // conversation-only rewind ("continue from here") without touching files.
                    if (mode === 'code') {
                    ...
                    try {
                        ...
                        json(response, 200, { status: 'completed', mode, sessionId: fork.sessionId, filesRestored: 0 });
                    }
                    catch (forkError) {
                        throw new ChangeLedgerError('RESTORE_FAILED', `conversation fork failed: ${errorMessage(forkError)}`, { cause: forkError });
                    }
                    return;
                }
