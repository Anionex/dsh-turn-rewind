 ...
 */
function lineageBoundary(session) {
...
async function createConversationRestart(ctx, sourceId, checkpoint) {
    ...
    if (apiProxy === undefined) {
        throw new ChangeLedgerError('CONVERSATION_REWIND_FAILED', 'this DSH host exposes neither sessionController nor apiProxy, so a new conversation cannot be created');
    }
...
async function sharedWorkspaceSessions(agents, cwd) {
    ...
    for (const agent of listed) {
        ...
        if (session.header.cwd === undefined)
            continue;
        try {
            if (await discoverWorkspaceRoot(session.header.cwd) === root)
