...
async function resolveTurnCheckpoint(ctx, engine, sessionId, turn) {
    ...
    while (true) {
        ...
        if (parentId === undefined || seedLength === undefined || boundary.seq >= seedLength)
            return undefined;
...
async function sharedWorkspaceSessions(agents, cwd, sourceSessionId) {
    ...
    for (const agent of listed) {
        ...
        if (session.id === sourceSessionId || session.header.cwd === undefined)
            continue;
        try {
