            ...
            try {
                ...
                await this.serializeWorkspace(workspace, captureSignal, async () => {
                    try {
                        ...
                        await this.recordFailure(ctx, agent.id, turn, checkpointTimeoutError(error, timeoutMs, captureDeadline, signal), { cwd: workspace, turnStartSeq: start.seq });
                    }
                });
            }
            catch (error) {
                await this.recordFailure(ctx, agent.id, turn, checkpointTimeoutError(error, timeoutMs, captureDeadline, signal), { cwd, turnStartSeq: start.seq });
            }
            finally {
