...
export class ChangeLedgerEngine {
    ...
    async planRestore(options) {
        ...
        try {
            ...
            await assertNoUnmanagedRestoreConflicts(source.state.root, desiredEntries, current.entries, selected.map((change) => change.path));
    ...
    async entriesForComparison(entries, workspace, signal) {
        if (!Object.values(entries).some((entry) => entry.kind === 'file' && entry.provider === 'git'))
            return entries;
        const normalized = Object.create(null);
        for (const [path, entry] of Object.entries(entries))
            normalized[path] = await this.entryForComparison(entry, workspace, signal);
        return normalized;
    }
    async entryForComparison(entry, workspace, signal) {
        if (entry?.kind !== 'file' || entry.provider !== 'git')
            return entry;
        const content = await readGitBlob(workspace, entry.blob, entry.size, signal);
        if (content.length !== entry.size)
            throw new ChangeLedgerError('BLOB_CORRUPT', `Git blob ${entry.blob} has unexpected size`);
