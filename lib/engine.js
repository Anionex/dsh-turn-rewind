...
import { readPathCache, writePathCache } from './path-cache.js';
...
export class ChangeLedgerEngine {
     ...
     */
    async pathCacheFor(workspaceDir) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // `auto` picks the fastest path each workspace can support: a Git
                    // worktree reuses the repository's own object database, while an
                    // ordinary directory can only use the ledger's store.
                    const gitNative = this.config.turnCheckpointMode === 'git-native'
                        || (this.config.turnCheckpointMode === 'auto' && source.state.type === 'git');
                    if (!gitNative) {
                        ...
                        try {
                            ...
                            // A Git-native turn tree cannot omit a path without changing the
                            // trust fences it is verified with, so a path that exceeds a limit
                            // falls back to the ledger's own store, which keeps the checkpoint
                            // and simply reports the paths it could not store.
                            if (!(error instanceof ChangeLedgerError)
    ...
    async recordTurnCheckpointSkip(options) {
        ...
        try {
            ...
            const existing = (await this.store.listManifests(workspace.root, options.signal)).some(manifest => manifest.kind === 'turn'
                && manifest.sessionId === options.sessionId
                && manifest.turn === options.turn
                && manifest.turnStartSeq === options.turnStartSeq);
            if (existing) {
    ...
    async findTurnCheckpointSkip(options) {
        ...
        const release = await this.acquireWorkspace(workspace, options.signal);
        try {
            const skip = await this.store.readTurnCheckpointSkip(workspace.root, options.sessionId, options.turn, options.turnStartSeq);
    ...
    async inspect(options) {
        ...
        try {
            ...
            return {
                ...
                changes: filterUnobservedChanges(manifest, diffTrees(manifest.entries, current.entries, manifest.entries, currentEntries)),
            };
        }
        finally {
            await release();
    ...
    async planRestore(options) {
        ...
        try {
            ...
            const changes = filterUnobservedChanges(manifest, diffTrees(manifest.entries, current.entries, manifest.entries, currentEntries));
            if (changes.length === 0) {
    ...
    async createLocked(options) {
        ...
        try {
            ...
            const manifest = {
                ...
                ...(tree.skipped.length === 0 ? {} : { skipped: tree.skipped }),
                ...(tree.skippedCount === 0 ? {} : { skippedCount: tree.skippedCount }),
                ...(tree.truncated === undefined ? {} : { truncated: tree.truncated }),
                restoreCount: 0,
            };
            if (options.deferGarbageCollection !== true) {
...
function summarize(manifest) {
    ...
    return {
        ...
        ...(manifest.skipped === undefined ? {} : { skipped: manifest.skipped }),
        skippedCount: manifest.skippedCount ?? 0,
        ...(manifest.truncated === undefined ? {} : { truncated: manifest.truncated }),
...
/**
 * Drop changes a restore point never observed.
 *
 * Limits leave a point partial by design, so it must never rewrite a path it
 * could not store, and while its skip list is incomplete it must not authorize
 * deleting a path it never saw either: a pre-existing file that merely exceeded
 * a limit must survive the rewind.
 * @param manifest - restore point being inspected or planned.
 * @param changes - raw path differences against the current workspace.
 * @returns the changes a restore may act on.
 */
function filterUnobservedChanges(manifest, changes) {
    const skipped = new Set((manifest.skipped ?? []).map(skip => skip.path));
    const incomplete = manifest.truncated !== undefined
        || (manifest.skippedCount ?? 0) > (manifest.skipped?.length ?? 0);
    if (skipped.size === 0 && !incomplete)
        return changes;
    return changes.filter(change => !skipped.has(change.path) && !(incomplete && change.kind === 'added'));
}
/** Reject a fence that now describes a different workspace or workspace mode. */
function assertWorkspaceIdentityCompatible(before, after) {
...
function assertRepositoryCompatible(manifest, current, allowHeadChange) {
    ...
    if (manifest.repository.type !== 'git' || current.type !== 'git')
        return;
...
function workspaceOperationChanged(before, after) {
    if (before.type !== 'git' || after.type !== 'git')
        return false;
...
function workspaceStaleError(before, after, phase) {
    if (before.type === 'git' && after.type === 'git') {
...
function checkpointMode(value) {
    if (value !== 'off' && value !== 'auto' && value !== 'git-native' && value !== 'legacy') {
        throw new ChangeLedgerError('INVALID_CONFIG', 'turnCheckpointMode must be off, auto, git-native, or legacy');
    }
    return value;
}
