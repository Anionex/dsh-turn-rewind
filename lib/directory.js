...
async function walkDirectory(root, directory, directoryPath, matcher, paths, maxFiles, signal) {
    throwIfAborted(signal);
    ...
    for (const entry of entries) {
        ...
        if (!info.isFile() && !info.isSymbolicLink()) {
            throw new ChangeLedgerError('UNSUPPORTED_FILE_TYPE', `ordinary-directory path is not a regular file or symlink: ${JSON.stringify(path)}`);
        }
        paths.push(path);
        if (paths.length > maxFiles) {
            throw new ChangeLedgerError('TOO_MANY_FILES', `workspace has more than ${maxFiles} eligible paths; latest path is ${JSON.stringify(path)}`);
        }
    }
}
/**
