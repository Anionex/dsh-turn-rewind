...
async function ensureWorktreeId(identityDir, commonDir, gitDir) {
    ...
    const paths = [identityPath, backupPath, initializedPath, centralPath];
    const copies = await Promise.all(paths.map(readIdentityCopy));
    const valid = copies.flatMap(copy => copy.value === undefined ? [] : [copy.value]);
    ...
    if (copies.some(copy => copy.malformed)) {
