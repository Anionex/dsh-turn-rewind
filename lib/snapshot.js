...
import { matchesPathCache, toPathCacheEntry } from './path-cache.js';
import { discoverWorkspace, sameWorkspaceFence } from './workspace.js';
...
export async function captureTree(options) {
    ...
    const recordSkip = (path, reason) => {
        skippedCount += 1;
        if (skipped.length < MAX_RECORDED_SKIPS)
            skipped.push({ path, reason });
        else
            truncated ??= reason === 'snapshot-limit' ? 'snapshot-limit' : 'file-limit';
    };
    ...
    };
    {
        ...
        const concurrency = Math.min(32, paths.length, Math.max(1, Math.floor(COMPARISON_READ_BUDGET_BYTES / options.config.maxFileBytes)));
...
function sameStagedPaths(left, right) {
    if (left.state.type !== 'git' || right.state.type !== 'git')
        return true;
...
async function captureEntry(root, path, maxFileBytes, signal) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        return {
            ...
            content,
            info: { ...before, size: BigInt(content.length) },
        };
    }
