...
async function responseJson(response) {
    ...
    try {
        ...
        throw new RewindRequestError(response.ok ? 'REWIND_INVALID_RESPONSE' : 'REWIND_ENDPOINT_UNAVAILABLE', `回退服务返回了无法解析的内容（HTTP ${String(response.status)}）。`);
    }
    ...
    if (!response.ok) {
        throw new RewindRequestError(typeof record?.code === 'string'
            ? record.code
            : response.status === 404 ? 'REWIND_ENDPOINT_UNAVAILABLE' : 'REWIND_FAILED', typeof record?.error === 'string' ? record.error : `请求失败：${String(response.status)}`);
    }
    if (value === undefined) {
