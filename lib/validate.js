...
export function parseManifest(value) {
    ...
    if (repository.type !== 'git')
        corrupt('Git-native v2 restore points must fence a Git workspace');
 ...
 */
function parseCaptureSkips(value) {
    if (value === undefined)
        return undefined;
    if (!Array.isArray(value))
        corrupt('skipped must be an array');
    ...
    if (record.type === 'directory') {
        for (const key of ['commonDir', 'head', 'branch', 'operation', 'stagedPaths']) {
            if (record[key] !== undefined)
                corrupt(`directory workspace state must not contain ${key}`);
        }
